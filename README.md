...
The Transactions page supports paginated search and filtering by account, category, type, and date range. A single compact, horizontally scrollable chip row provides immediate date-range, account, and transaction-type choices while keeping the transaction list high on the page. Category remains directly accessible in a select beside the chips (or immediately below them on mobile). The Filters button opens an accessible right-side sheet containing search, custom dates, amounts, sorting, totals month, and complete account/type controls; its badge shows the active-filter count. Quick date ranges cover the current and previous month, the last 30 days, current quarter, year to date, and all time. Filter changes remain URL-driven and reset pagination. Manual transactions can be created, edited, categorized, annotated, and soft-deleted; deleting from a filtered listing preserves the active filters. Amounts are signed account movements: income and refunds are positive, expenses are negative, and transfers may use either sign.

...
The dashboard opens with a full-width safe-to-spend summary blended into the page background. Its allowance guide visualizes the positive remaining amount declining toward zero at month end; it is a planning guide rather than recorded daily spending. Negative results use a shortfall treatment instead. The current-month supporting metrics follow immediately below the summary, and the complete derivation remains available from the hero.

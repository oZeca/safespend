...
The Transactions page supports paginated search and filtering by account, category, included or excluded transaction types, and date range, with sorting by date or signed transaction amount in either direction. The income, expense, and savings summary reflects the active date range, or all time when no date bounds are selected. A single compact, horizontally scrollable chip row provides immediate date-range, account, and transaction-type choices while keeping the transaction list high on the page. Category remains directly accessible in a select beside the chips (or immediately below them on mobile). The Filters button opens an accessible right-side sheet containing search, custom dates, amounts, sorting, transaction-type exclusions, and complete account/type controls; its badge shows the active-filter count. Quick date ranges cover the current and previous month, the last 30 days, current quarter, year to date, and all time. Filter changes remain URL-driven and reset pagination. Manual transactions can be created, edited, categorized, annotated, and soft-deleted; deleting from a filtered listing preserves the active filters. Amounts are signed account movements: income and refunds are positive, expenses are negative, and transfers may use either sign.
...
The dashboard is organized into URL-backed Overview, Spending, and Wealth views. Overview opens with a full-width safe-to-spend summary blended into the page background. Its allowance guide visualizes the positive remaining amount declining toward zero at month end; it is a planning guide rather than recorded daily spending. Negative results use a shortfall treatment instead. The current-month supporting metrics, savings-target status, immediate forecast assumptions, and complete derivation remain on Overview.

The dashboard reports available cash from active accounts opted into that total, current-month income, net expenses after refunds, current-month savings, and year-to-date savings. Transfers and deleted transactions are excluded from actuals. Spending opens with the current-year income, expense, and savings chart in an edge-to-edge gradient hero, followed by current-period metrics and category analysis. Wealth opens with the actual and projected net-worth chart in the same integrated hero treatment, followed by current cash, investment, and net-worth totals.

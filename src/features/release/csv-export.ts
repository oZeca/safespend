...
interface ExportRow {
  ...
  category: string | null; notes: string | null; isRecurring: number; isExceptional: number; excludedFromForecastBaseline: number;
...
export function exportTransactionsCsv(database: Database.Database): string {
  const rows = database.prepare(`SELECT t.id, t.date, a.name AS account, t.description, t.merchant, t.amount_cents AS amountCents,
    ...
    t.excluded_from_forecast_baseline AS excludedFromForecastBaseline, t.source_import_id AS sourceImportId, t.source_row_number AS sourceRowNumber
  ...
  const headers = ["id", "date", "account", "description", "merchant", "amount", "type", "category", "splits_json", "notes", "recurring", "exceptional", "excluded_from_forecast_baseline", "transfer_counterpart_id", "source_import_id", "source_row_number"];
  ...
  for (const row of rows) {
    ...
    lines.push([
      ...
      splitJson, row.notes, row.isRecurring, row.isExceptional, row.excludedFromForecastBaseline, counterpart ?? null, row.sourceImportId, row.sourceRowNumber
    ].map(csvCell).join(","));
  }
  return `${lines.join("\r\n")}\r\n`;
}

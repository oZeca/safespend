...
export function AccountForm({ action, account }: { action: FormAction; account?: Account }) {
  ...
  const initialBalanceMode = value("balanceMode", account?.balanceMode ?? "manual") as "manual" | "calculated";
  const [balanceMode, setBalanceMode] = useState(initialBalanceMode);
  ...
  return (
    <form action={formAction} className="space-y-6 rounded-xl border bg-card p-5 shadow-sm sm:p-7">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div className="space-y-2 sm:col-span-2"><label className="block space-y-2"><span className="text-sm font-medium">Balance mode</span><select className={inputClass} name="balanceMode" onChange={(event) => setBalanceMode(event.currentTarget.value as "manual" | "calculated")} value={balanceMode}><option value="manual">Manual balance</option><option value="calculated">Calculated from transactions</option></select></label><p className="text-xs text-muted-foreground">Calculated balance equals the opening balance plus signed transactions from the opening date, excluding transactions marked as internal movements.</p></div>
        {balanceMode === "manual" ? <label className="space-y-2 sm:col-span-2"><span className="text-sm font-medium">Current balance</span><div className="relative"><span className="absolute left-3 top-2 text-muted-foreground">€</span><input className={`${inputClass} pl-8`} defaultValue={value("currentBalance", account ? formatMoneyInput(account.manualBalanceCents) : "0.00")} inputMode="decimal" name="currentBalance" required /></div><p className="text-xs text-muted-foreground">Use a negative value for money owed.</p><FieldError errors={state.errors?.currentBalance} /><input name="openingBalance" type="hidden" value={value("openingBalance", account ? formatMoneyInput(account.openingBalanceCents) : "0.00")} /><input name="openingBalanceDate" type="hidden" value={value("openingBalanceDate", account?.openingBalanceDate ?? localDateString())} /></label>
          : <><input name="currentBalance" type="hidden" value={value("currentBalance", account ? formatMoneyInput(account.manualBalanceCents) : "0.00")} /><label className="space-y-2"><span className="text-sm font-medium">Opening balance</span><div className="relative"><span className="absolute left-3 top-2 text-muted-foreground">€</span><input aria-label="Opening balance" className={`${inputClass} pl-8`} defaultValue={value("openingBalance", account ? formatMoneyInput(account.openingBalanceCents) : "0.00")} inputMode="decimal" name="openingBalance" required /></div><p className="text-xs text-muted-foreground">Balance before transactions on the opening date.</p><FieldError errors={state.errors?.openingBalance} /></label><label className="space-y-2"><span className="text-sm font-medium">Opening date</span><input className={inputClass} defaultValue={value("openingBalanceDate", account?.openingBalanceDate ?? localDateString())} name="openingBalanceDate" required type="date" /><FieldError errors={state.errors?.openingBalanceDate} /></label></>}
      </div>

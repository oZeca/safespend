...
function Submit() { const { pending } = useFormStatus(); return <Button disabled={pending} type="submit">{pending ? "Reading CSV…" : "Upload and continue"}</Button>; }
export function UploadForm({ accounts, profiles }: { accounts: AccountOption[]; profiles: ImportProfile[] }) {
  ...
  return <form action={action} className="space-y-5 rounded-xl border bg-card p-5 shadow-sm">
    ...
    <label className="block space-y-2"><span className="text-sm font-medium">CSV file</span><input accept=".csv,text/csv" className="block w-full rounded-md border bg-background p-2 text-sm" name="file" required type="file" />{state.errors?.file && <p className="text-sm text-red-700">{state.errors.file[0]}</p>}</label>

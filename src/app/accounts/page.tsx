...
function AccountCard({ account }: { account: Account }) {
  return (
    <article className="flex flex-col gap-4 rounded-xl border bg-card p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex items-center justify-between gap-4 sm:justify-end">{account.isArchived || account.balanceMode === "calculated" ? <div className="text-right"><p className="text-xl font-semibold tabular-nums">{formatCurrency(account.currentBalanceCents, account.currency)}</p>{account.balanceMode === "calculated" && <p className="text-xs text-muted-foreground" data-sensitive>{formatCurrency(account.openingBalanceCents, account.currency)} opening + {formatCurrency(account.currentBalanceCents - account.openingBalanceCents, account.currency)} transactions since {account.openingBalanceDate}</p>}</div> : <InlineBalanceInput accountName={account.name} action={updateAccountBalanceAction.bind(null, account.id)} balanceCents={account.currentBalanceCents} currency={account.currency} />}
